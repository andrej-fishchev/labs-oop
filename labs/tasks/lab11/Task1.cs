...
public sealed class Task1 : LabTask
{
        ...
        base(name, description)
    {
        ...
        Actions = new List<ILabEntity<string>>()
        {
            ...
            new LabTaskActionBuilder().Name("Выполнить клонирование")
                .ExecuteAction(UseClone)
                .Build(),
            ...
            new LabTaskActionBuilder().Name("Поиск элемента")
    ...
    public void InputData(ArrayGenerationType type)
    {
        ...
        if (type == ArrayGenerationType.UserInput)
        {
            ...
            if (data) Times = ConsoleResponseDataFactory
                    .MakeResponse(new ListT<Time>(data.Data()));
    ...
    {
        Times.Data((ListT<Time>)Times.Data().Clone());
    }

    public void SortElements()
    {
        Time[] times = Times.Data().ToArray();
        
        Array.Sort(times, (obj, obj2) =>
        {
            if (obj.AsSeconds() == obj2.AsSeconds())
                return 0;

            return (obj.AsSeconds() > obj2.AsSeconds()) ? 1 : -2;
        });

        Times.Data(new ListT<Time>(times));
    }
    ...
    public void SearchElement()
    {
        ...
        int i = Array.BinarySearch(Times.Data().ToArray(), buffer.Data());
        
        if(i > -1)
            Target.Output.WriteLine($"Объект: {buffer.Data()} найден под номером {i + 1}");
    }
